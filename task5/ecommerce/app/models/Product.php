...
{
    private $id, $name_en, $name_ar, $quantity,
     $price, $details_en, $details_ar, 
     $code, $image, $status,
    ...
    public function getProducts($filter = null, $order = null)
    {
        $query = "SELECT * FROM `products_details` WHERE `status` = {$this->status} $filter $order";
    ...
    public function mostOrderProduct()
    {
        $query = "SELECT
        `products`.*,
        `order_product`.`product_id` AS `order_product_id`, 
         COUNT(`order_product`.`product_id`) AS `orders_count`
    FROM
        `products`
    LEFT join `order_product`
    on `products`.`id` = `order_product`.`product_id`   
    GROUP BY `products`.`id`
    ORDER BY `orders_count` DESC LIMIT 4
        ";
        return $this->runDQL($query);
    }
}
